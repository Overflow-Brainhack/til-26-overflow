...
class EditedHeuristicPolicy(Policy):
    ...
    def __init__(
        ...
        *,
        ...
        self.minimum_aggression = minimum_aggression
        self.aggression_ramp_rate = aggression_ramp_rate
        self.defensive_force = defensive_force
        ...
        self._aggression_floor: float = minimum_aggression
    ...
    def choose(self, obs: ParsedObs, memory: MapMemory) -> int:
        ...
        my_blast_tick = imminent_danger(memory, obs.location, timeline)
    ...
    def _bomb_opportunity_score(
        ...
        agent_hits = sum(
            ...
            if p in blast and not self._enemy_can_escape_blast(p, blast, memory)
        )
    ...
    def _try_attack(self, obs: ParsedObs, memory: MapMemory) -> Optional[int]:
        ...
        if self.bomb_economy:
            ...
            direct_base_hit = any(p in blast for p in memory.enemy_bases)
            high_confidence = score >= max(
                self.bomb_reserve_threshold,
                self.base_bomb_value,
            )
            if direct_base_hit or high_confidence:
    ...
    def _enemy_can_escape_blast(
        ...
        """Best-case enemy escape test for a bomb blast.

        Enemy facing is not observed, so this tries all four possible facings
        and returns True if any orientation can reach a cell outside the blast
        before detonation. The movement model includes turning because
        reachable_cells searches the same (position, facing) action graph used
        by our own policy.
        """
        if enemy_pos not in blast:
            return True

        def edge(a: tuple[int, int], b: tuple[int, int]) -> Optional[float]:
            if not memory.in_bounds(b):
                return None
            return 1.0 if memory.passable(a, b) else None
        ...
        for facing in Direction:
            reachable = reachable_cells(
                ...
                int(facing),
                edge,
                max_cost=float(BOMB_TIMER),
            )
            if any(cell not in blast for cell in reachable):
                return True
        return False
    ...
    def _try_defend(
        ...
        effective_radius = max(1, int(round(effective_radius * self.defensive_force)))
        ...
        if not combined_threats:
            return None
        if self._defense_too_late(obs, memory, combined_threats):
            return None
    ...
    def _defense_too_late(
        self,
        obs: ParsedObs,
        memory: MapMemory,
        threats: set[tuple[int, int]],
    ) -> bool:
        """Skip defense when the agent is too far from home to matter.

        On the novice map this prevents a base-rushing/collection agent from
        abandoning a productive attack route when an enemy is already closer to
        our base than we are.  Concrete threats still trigger defense when we
        are within max_defense_distance of base or can plausibly beat the enemy
        back by defense_abandon_margin cells.
        """
        if memory.ally_base is None:
            return False
        if self.max_defense_distance <= 0:
            return False

        bx, by = memory.ally_base
        my_to_base = abs(obs.location[0] - bx) + abs(obs.location[1] - by)
        if my_to_base <= self.max_defense_distance:
            return False

        threat_to_base = min(abs(p[0] - bx) + abs(p[1] - by) for p in threats)
        return my_to_base > threat_to_base + self.defense_abandon_margin
    ...
    def _effective_base_weight(self) -> float:
        ...
        return max(base, self._aggression_floor)
    ...
    def _update_adaptive_weight(self, obs: ParsedObs, memory: MapMemory) -> None:
        ...
        """
        self._aggression_floor = min(
            self.base_route_weight,
            self.minimum_aggression + obs.step * self.aggression_ramp_rate,
        )
        ...
        if obs.step == 0:
            ...
            self._aggression_floor = self.minimum_aggression
    ...
    def _maybe_wall_break(
        ...
        take a short collectible detour when one is available; otherwise STAY
        and let it detonate instead of wasting another bomb.
        """
        ...
        if not self.wall_breaking:
            return None  # path scoring should not have produced this — reject

        sitting_bomb = memory.bombs.get(obs.location)
        if sitting_bomb is not None and sitting_bomb.ally:
            move = self._productive_wait_move(obs, memory)
            if move is not None:
                return move
            return int(Action.STAY)
    ...
    def _productive_wait_move(
        self,
        obs: ParsedObs,
        memory: MapMemory,
    ) -> Optional[int]:
        """Use the wall-break fuse on a short, returnable collectible detour."""
        radius = _PRODUCTIVE_WALL_WAIT_RADIUS
        if radius <= 0:
            return None
        ...
        for cell in candidates:
            ...
            if dist is None or dist <= 0 or dist > radius:
                continue
        ...
        move = first_action_to(obs.location, obs.direction, {best_cell}, edge)
        if move is None or move == Action.STAY:
            return None
        if move in (Action.FORWARD, Action.BACKWARD):
            dest = next_pos_after(obs.location, obs.direction, move)
            if not memory.passable(obs.location, dest):
                return None
        return int(move)
