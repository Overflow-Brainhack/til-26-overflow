"""Azbase v4 — v1 with collected-tile cooldown and dead-base filtering.

Preserved from the `overflow-ae:azbasev4` Docker image. Eval scores: 0.718,
0.724.

What v4 adds over [[azbasev1_policy]]:
- **Collected-tile cooldown** (35 steps): instead of treating a tile as
  permanently visited after one collection, the agent re-considers it once
  ``collected_tile_cooldown`` steps have passed. Helps with respawning tile
  types that v1 would ignore for the rest of the round.
- **Dead-base filtering**: ``_ordered_bases`` filters out enemy bases whose
  ``enemy_base_health`` has hit zero, so the agent doesn't waste a route
  attacking a corpse. Requires importing ``BASE_MAX_HEALTH``.

All other priorities (proactive base routing, weakest-base-first,
resource refill bias when bombs are low) are inherited from v1's recipe.
"""
...
class AzbaseV4Policy(EditedHeuristicPolicyV2):
