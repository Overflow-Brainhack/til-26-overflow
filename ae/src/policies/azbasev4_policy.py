"""Berserker Base policy as shipped in the ``overflow-ae:azbasev4`` image.

Unfolded from ``azbasev4_preserved/berserker_base_policy.py`` (class renamed
to ``BerserkerBaseV4Policy`` for unambiguous local comparisons; adds the
collected-tile cooldown / dead-base filtering on top of v3). Only the two
azbase imports were re-pointed at sibling modules in this package. See
``policies/README.md`` for the full snapshot provenance.
"""
...
class BerserkerBaseV4Policy(EditedHeuristicPolicyV2):
