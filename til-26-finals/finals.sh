...
assert_team_and_task() {
  if [ -z "$TEAM_NAME" ]; then
    echo "No team name found in environment!"
    exit 1
  fi
  if [ -z "$1" ]; then
...
ensure_task_finals_image() {
  ...
  cwd=$(pwd)
  cd "$til_folder/$task" || {
    echo "ERROR: could not enter '$til_folder/$task'."
    exit 1
  }
...
push_task_finals_image() {
  ...
  docker tag "$image" "$ar_ref" || {
    echo "ERROR: 'docker tag' failed for $image."
    exit 1
  }
  echo "Pushing '$ar_ref' to Artifact Registry..."
  docker push "$ar_ref" || {
    echo "ERROR: 'docker push' failed for $ar_ref."
    exit 1
  }
  ...
  gcloud ai models upload --region asia-southeast1 --display-name "$TEAM_NAME-$task" \
    ...
    {
      echo "ERROR: 'gcloud ai models upload' failed for $TEAM_NAME-$task."
      exit 1
    }
}
...
build)
  ...
  cwd=$(pwd)
  cd "$til_folder/$task" || {
    echo "Could not find directory for task '$task'! Are you sure you have a directory named '$task' in your 'til-26' directory?"
    til_folder_warning
    cd "$cwd"
    exit 1
  }
